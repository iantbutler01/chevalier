...
/// Advisory-lock namespace carried by the FUSE protocol.
///
/// POSIX byte-range locks and BSD `flock(2)` locks have distinct ownership and
/// conflict domains. Filesystems that forward locks must preserve this value
/// instead of inferring it from the range or owner.
...
/// Result of delivering a kernel interrupt to a filesystem.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum InterruptResult {
    /// The target request was found and cancellation was initiated.
    ///
    /// Fuser sends no reply for the interrupt request itself. The filesystem
    /// remains responsible for completing the original request, normally with
    /// [`Errno::EINTR`].
    Handled,
    /// The target request is not currently known to the filesystem.
    ///
    /// Fuser replies to the interrupt request with [`Errno::EAGAIN`] so the
    /// kernel can requeue it if the original request has not arrived yet.
    Unhandled,
}
...
pub trait Filesystem: Send + Sync + 'static {
    ...
    ///
    /// `request_id` identifies the original request the kernel wants to
    /// cancel, not this interrupt request (whose id remains available through
    /// `req`). Return [`InterruptResult::Handled`] after signaling the target
    /// operation; return [`InterruptResult::Unhandled`] when the target is not
    /// registered so the kernel can retry the interrupt.
    fn interrupt(&self, _req: &Request, _request_id: RequestId) -> InterruptResult {
