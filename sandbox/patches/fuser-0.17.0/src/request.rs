...
fn dispatch_interrupt<FS: Filesystem>(
    ...
    match filesystem.interrupt(request, request_id) {
        InterruptResult::Handled => Ok(None),
        InterruptResult::Unhandled => Err(Errno::EAGAIN),
    }
}
...
mod tests {
    ...
    struct RecordingFilesystem {
        ...
        interrupt_request: AtomicU64,
        target_request: AtomicU64,
    }

    impl Filesystem for RecordingFilesystem {
        fn interrupt(&self, req: &Request, request_id: RequestId) -> InterruptResult {
            self.interrupt_request
                .store(req.unique().0, Ordering::SeqCst);
            self.target_request.store(request_id.0, Ordering::SeqCst);
            if request_id == self.handled_request {
    ...
    #[test]
    fn interrupt_dispatch_suppresses_handled_reply_and_requeues_unknown_request() {
        let header = fuse_in_header {
            len: size_of::<fuse_in_header>() as u32,
            opcode: fuse_opcode::FUSE_INTERRUPT as u32,
            unique: 41,
            nodeid: 0,
            uid: 0,
            gid: 0,
        ...
        let request = Request::ref_cast(&header);
        let filesystem = RecordingFilesystem {
            handled_request: RequestId(99),
            interrupt_request: AtomicU64::new(0),
            target_request: AtomicU64::new(0),
        };

        assert!(matches!(
            dispatch_interrupt(&filesystem, request, RequestId(99)),
            Ok(None)
        ));
        assert_eq!(
            filesystem.interrupt_request.load(Ordering::SeqCst),
            request.unique().0
        );
        assert_eq!(filesystem.target_request.load(Ordering::SeqCst), 99);

        assert_eq!(
            dispatch_interrupt(&filesystem, request, RequestId(100))
                .unwrap_err()
                .code(),
            Errno::EAGAIN.code()
        );
