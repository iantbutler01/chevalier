...
func (n *node) tryWLockPathWait(wait bool) (acquired bool, blocker *node) {
	if n == nil {
	...
	if blocker := n.parent.tryRLockPath(wait); blocker != nil {
...
func (n *node) tryWLockPath() bool {
	acquired, _ := n.tryWLockPathWait(false)
	return acquired
}
...
func (n *Node) WLockPath() *PathLock {
	for {
		result, waitCh := func() (*PathLock, chan struct{}) {
			n.locker.mtx.Lock()
			defer n.locker.mtx.Unlock()
			acquired, blocker := n.node.tryWLockPathWait(true)
			if !acquired {
				if blocker == nil {
					return nil, nil
				}
...
func (tl *TreeLocker) wlockClean(p string) *PathLock {
	for {
		result, waitCh := func() (*PathLock, chan struct{}) {
			...
			node := tl.allocRetainClean(p)
			defer node.free()
			acquired, blocker := node.tryWLockPathWait(true)
			if !acquired {
				if blocker == nil {
					return nil, nil
				}
