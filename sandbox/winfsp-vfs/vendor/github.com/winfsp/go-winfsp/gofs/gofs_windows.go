...
func (fs *fileSystem) SetBasicInfo(
	...
	defer handle.unlockChecked()
	setter, ok := handle.file.(FileSetBasicInfo)
	if !ok {
		return windows.STATUS_NOT_SUPPORTED
	}
	if err := setter.SetBasicInfo(
		flags, attribute,
...
type FileSetBasicInfo interface {
	File
