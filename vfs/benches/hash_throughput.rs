...
fn main() {
    ...
    println!("buffer: {} MiB, threads available: {}", size / (1024 * 1024), std::thread::available_parallelism().map(|n| n.get()).unwrap_or(1));
    ...
    println!("sha256 (sha2 crate)      {:>8.0} MB/s   {:.2}s", mb_per_s(size, e), e);

    let t = Instant::now();
    let _ = blake3::hash(&data);
    let e = t.elapsed().as_secs_f64();
    println!("blake3 (single-thread)   {:>8.0} MB/s   {:.2}s", mb_per_s(size, e), e);
    ...
    let _ = h.finalize();
    let e = t.elapsed().as_secs_f64();
    println!("blake3 (multi-thread)    {:>8.0} MB/s   {:.2}s", mb_per_s(size, e), e);
}
